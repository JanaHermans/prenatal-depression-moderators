########################################################################
# Title:    Main analyses with full imputed sample (ALSPAC)
# Author:   Jana Hermans
...
libraries <- c("haven","lme4","dplyr","mice","ggplot2","grid","gridExtra",
...
imputed.dfs <- lapply(imputed.dfs, function (x)
  x <- x %>%
    dplyr::mutate(preg_dep = dplyr::case_when(
      ...
      !is.na(preg_dep18w) &  !is.na(preg_dep32w) ~ rowMeans(x[,c('preg_dep18w','preg_dep32w')])# Otherwise take mean
    ))
)
...
#imputed.dfs <- lapply(imputed.dfs, function (x)
  #x <- x %>%
    #dplyr::mutate(friendship_f = dplyr::case_when(
      #friendship == 1 ~ "4: Very happy",
      #friendship == 2 ~ "3: Quite happy",
      #friendship == 3 ~ "2: Quite unhappy",
      #friendship == 4 ~ "1: Unhappy",
      #TRUE ~ NA_character_
    #))
#)
...
imputed.dfs <- lapply(imputed.dfs, function(x) {
  ...
  x$exter_z <- as.numeric(x$exter_z) %>%
    as.vector()
  x$friendship_z <- as.numeric(x$friendship) %>%
    scale()
...
saveRDS(res, file=file.path(projectpath,"output", 
                            paste0('res_main_imputed_ALSPAC',
...
ggsave(paste0(projectpath, "/output/model_checks/Residuals_vs_Fitted_ALSPAC.png"), 
       big_resid_plot, width = 20, height = 18, dpi = 300)
ggsave(paste0(projectpath, "/output/model_checks/Residual_Histograms_ALSPAC.png"), 
       big_resid_hist, width = 20, height = 18, dpi = 300)
ggsave(paste0(projectpath, "/output/model_checks/QQ_Plots_ALSPAC.png"), 
...
saveRDS(res, file=file.path(projectpath,"output", 
                            paste0('res_mod_stratified_imputed_ALSPAC',
                                   as.character(Sys.Date()),".Rdata")))
...
saveRDS(res_direct, file=file.path(projectpath,"output", 
                            paste0('res_direct_imputed_ALSPAC',
                                   as.character(Sys.Date()),".Rdata")))
...
saveRDS(res, file=file.path(projectpath,"output", 
                            paste0('res_prepregdep_imputed_ALSPAC',as.character(Sys.Date()),".Rdata")))
...
saveRDS(res, file=file.path(projectpath,"output", 
                            paste0('res_continuous_imputed_ALSPAC',as.character(Sys.Date()),".Rdata")))
