...
data <- load_alspac(file.path(datapath, filename), # filepath or nothing (for interactive window)
  lower.case = TRUE,
...
#idc <- make_idc(mom.id='cidb4611')
IDC <- paste(data$cidb4611, # mother ID
                                gsub('\\s+', '', data$qlet), # sibling ID (A or B)
                                sep = "_") # --> format "1_A" 
# Initiate dataframe and set up unique child ID
#idc2 <- paste(idc, # mother ID
...
# ==============================================================================
# Wrap this up and copute total score, leave PR_prenatal out
prenatal_stress <- cbind(IDC, LE_prenatal, CR_prenatal, IR_prenatal)
...
# ==============================================================================
# Wrap this up and compute total score, leave PR_prenatal out
postnatal_stress <- cbind(IDC, LE_postnatal, CR_postnatal, IR_postnatal, DV_postnatal)
...
#-----------------------------------3) SAVE-------------------------------------
saveRDS(prenatal_stress, file.path(datapath,'ALSPAC_pre_ELS.rds'))
saveRDS(postnatal_stress, file.path(datapath,'ALSPAC_post_ELS.rds'))
#save(prenatal_stress, postnatal_stress, file='ELS_score_ALSPAC.RData')
